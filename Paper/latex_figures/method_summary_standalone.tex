...
\begin{tikzpicture}[
    ...
    \node[box] (helm) {%
        ...
        \eq{\bigl[\nabla^2 + (2\pi k_0)^2\,n^2(\mathbf{r})\bigr]\;\psi(\mathbf{r})=0}\\[12pt]
    ...
    \node[box, right=\boxgap of ode] (wpm) {%
        ...
        \eqtight{\psi_m=\mathcal F^{-1}\!\left[e^{2\pi i\Delta z(\sqrt{k_0^2n_m^2-k_\perp^2}-k_0)}\mathcal F\{\psi(z)\}\right]}\\[8pt]
    ...
    \node[box, right=\boxgap of wpm] (as) {%
        ...
        \eqtight{\psi(z\!+\!\Delta z)\approx e^{i\sigma V_{\mathrm{proj}}}\,\mathcal F^{-1}\!\left[e^{2\pi i\Delta z(\sqrt{k_0^2-k_\perp^2}-k_0)}\mathcal F\{\psi(z)\}\right]}\\[8pt]
    ...
    \draw[semithick, ->] ($(ode.south east)+(0,-\derivdrop)$) -- ($(wpm.south west)+(0,-\derivdrop)$)
        node[midway, below=2pt, deriv]{one-way branch plus Schmidt WPM approximation\\$\sqrt{k_0^2n^2+\nabla_\perp^2/(4\pi^2)}\to\sqrt{k_0^2n_m^2-k_\perp^2}$};
