...
def reconstruct_potential_1d(
    ...
    *,
    ...
    """Recover independent non-negative potential pixels inside a fixed mask."""
    try:
        import optax
    ...
    max_phase = float(np.asarray(interaction_constant(energy))) * float(
        np.asarray(slice_thickness)
    ) * resolved_max
    if max_phase >= np.pi:
        raise ValueError(
    ...
    def assemble(normalized_values: Array) -> Array:
        return _scatter_masked_values(normalized_values, flat_indices, (n_s, n_u), scale)
