...
def test_pixel_reconstruction_keeps_fixed_exterior_values():
    pytest.importorskip("optax", reason="the ptychography extra is not installed")
    n_s, n_u = 5, 12
    du, ds = 0.3, 0.4
    mask = jnp.zeros((n_s, n_u), dtype=bool).at[1:4, 4:8].set(True)
    fixed = jnp.full((n_s, n_u), 25.0)
    initial = jnp.zeros((n_s, n_u))
    probe = _probe(n_u=n_u, du=du)
    kernel = _kernel(n_u=n_u, du=du, ds=ds)
    starts = jnp.asarray([0, 1])
    measured = simulate_glancing_scan_1d(
        fixed, probe, starts, 4, kernel, ds, ENERGY
    )
    ...
    np.testing.assert_array_equal(
        ...
        np.asarray(fixed)[~np.asarray(mask)],
    )
    assert len(result.elapsed_time_history) == len(result.update_history)
